...
public class ModuleCommand extends Command {
    ...
    public void execute(String[] args) {
        if (args.length >= 2) {
            IngrosWare.INSTANCE.getSettingManager().getSettingsFromObject(module).forEach(property -> {
                if (args[1].equalsIgnoreCase(property.getLabel())) {
                    if (property instanceof BooleanSetting) {
                        ...
                        Logger.printMessage(booleanProperty.getLabel() + " has been " + (booleanProperty.getValue() ? ChatFormatting.PREFIX_CODE + "aenabled" + ChatFormatting.PREFIX_CODE + "7" : ChatFormatting.PREFIX_CODE + "cdisabled" + ChatFormatting.PREFIX_CODE + "7") + " for " + module.getLabel() + ".");
                        ...
                        if (args.length >= 3) {
                            if (args[2].equalsIgnoreCase("help")) {
                                ...
                                Logger.printMessage(property.getLabel() + " has been set to " + property.getValue() + " for " + module.getLabel() + ".");
                            }
                        } else {
